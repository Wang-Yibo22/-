...
//	Read the control line and the dimension of the problem
	Input >> NUMNP >> NUMEG >> NLCASE >> MODEX >> Dimontion ;
...
//	Loop over for all nodal points
    ...
    {
		if (!NodeList[np].Read(Input,Dimontion))
			return false;
...
//		Loop over for all elements in group EleGrp
        ...
        {
            CElement& Element = ElementGrp[Ele];
            Element.ElementStiffness(Matrix); // Calculate element stiffness matrix
